...
def broadcast_to_client():
    ...
    while True:
        if event.is_set():
            break
        if not json_send_queue.empty():
            data = json_send_queue.get()
            #print("SEND:", data)
...
def main():
    ...
    server = Thread(target=server_start_callback, daemon=True)
    server.start()

    send_client_thread = Thread(target=broadcast_to_client)
